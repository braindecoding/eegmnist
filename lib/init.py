...
def randombetween0and1withmatrixsize(numTrn,K,C,D2):
    Z_mu = np.asmatrix(random.random(size=(numTrn,K))).astype(np.float32)
    B_mu = np.asmatrix(random.random(size=(K,D2))).astype(np.float32)
    R_mu = np.asmatrix(random.random(size=(numTrn,C))).astype(np.float32)
    H_mu = np.asmatrix(random.random(size=(C,D2))).astype(np.float32)
    return Z_mu,B_mu,R_mu,H_mu

def matriksidentitasukuran(C):
    sigma_r = np.asmatrix(np.eye((C))).astype(np.float32)
    sigma_h = np.asmatrix(np.eye((C))).astype(np.float32)
